...
for (id in names(resLST)) {
  ...
  dat_list[[id]] <- vector("list", 13)
  names(dat_list[[id]]) <- c("N", "P", "M", "IDp", "IDs", "AGE", "SEX", "APOE4", "time", "S", "PBLHVLeap", "PBLHVFS", "ASFVAL")
  dat_list[[id]]$N <- N
  dat_list[[id]]$P <- P
  dat_list[[id]]$M <- M
  ...
  dat_list[[id]]$PBLHVLeap <- (resLST[[id]]$PBLHVTOTAL_LEAP - mean(resLST[[id]]$PBLHVTOTAL_LEAP)) / sd(resLST[[id]]$PBLHVTOTAL_LEAP)
