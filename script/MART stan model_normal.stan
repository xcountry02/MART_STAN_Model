data{
  ...
  vector[N] time;      			        // time of observation (years)
	vector[N] S;           			      // measured ADAScog or CDRSB score
 	vector[N] PBLHV;           			  // measured normalized Hippocampus Volume
...
model{
	...
	//Priors
    	omega_pb~normal(0,10); 
    	omega_pr~normal(0,10);
    	eta_pb~normal(0,omega_pb);
    	eta_pr~normal(0,omega_pr);

    	theta_S0~normal(0,10); 
    	theta_r~normal(0,10);
    	theta_SEX~normal(0,10);
    	theta_AGE~normal(0,10);
    	theta_HV~normal(0,10);
    	theta_APOE4_b~normal(0,10);
      theta_APOE4_r~normal(0,10);
    	tau~normal(0,10); 
    	beta~normal(0,10);
